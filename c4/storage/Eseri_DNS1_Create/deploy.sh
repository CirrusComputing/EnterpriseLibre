#!/bin/bash
#
# Eseri central DNS configuration script (before org DNS) - v1.6
#
...
host c3.$SYSTEM_ANCHOR_DOMAIN | awk '/^.*has address [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/ {print $4}' > $RESULT_FOLDER/Eseri_C3_IP.txt
host $(hostname -f) | awk '/^.*has address [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/ {print "ESERI_DNS_IP_PRIVATE="$4}' >> $RESULT_FOLDER/Eseri_DNS_IP.txt
host $(dig @8.8.8.8 +short SOA $SYSTEM_ANCHOR_DOMAIN | awk '{print $1}') 8.8.8.8 | awk '/^.*has address [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/ {print "ESERI_DNS_IP_PUBLIC="$4}' >> $RESULT_FOLDER/Eseri_DNS_IP.txt
