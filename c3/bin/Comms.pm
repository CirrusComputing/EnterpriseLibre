...
# Also contains some other common functions.
#
...
@ISA         = qw(Exporter);
@EXPORT      = ();
@EXPORT_OK   = qw(ssh scp ssh_key get_system_anchor_domain);
%EXPORT_TAGS = ( DEFAULT => [qw(ssh scp ssh_key get_system_anchor_domain)],
                 Both    => [qw(ssh scp)]);
...
sub get_db_conn{
    my $conf = new Config::General("$ENV{HOME}/bin/xml_rpc.config");
